import type { ChatType, Message, SendType } from '../../red'
import { MsgType } from '../../red'
 ...
 * # 消息类型
 *
 * QQNT 的消息类型共有四种，它们分别是 {@link Message.chatType}、{@link Message.msgType}、{@link Message.subMsgType}
 * 和 {@link Message.sendType}。下面分别介绍这四种类型。
 *
 * ## {@link Message.chatType}
 *
 * 消息的归属。目前只有 {@link ChatType.Private} 和 {@link ChatType.Group}。
 *
 * ## {@link Message.msgType}
 *
 * 消息的主要类型。
 *
 * ## {@link Message.subMsgType}
 *
 * 消息内容中包含的元素类型。普通消息的情况下以 Bitset 提供，可使用 {@link adaptMsgTypes}
 * 解析为一组 {@link Boolean} 字段。
 *
 * ## {@link Message.sendType}
 *
 * 消息的呈现类型。
 */
 ...
 * 解析 QQNT 消息类型。
 */
export const parseMsgTypes = (message: Message): MsgTypes => ({
